...
\begin{itemize}
  \item from model to probability: $p=\frac{1}{1+\exp(-x)}$
...
\begin{column}{0.5\textwidth}
\begin{itemize}
  \item Random effect variance is distorted by logit
  \item AND lots of variance from random process on top of expected values
\end{itemize}
\end{column}
\end{columns}
