...
<<>>=
survprob5 <- 1/(1+exp(-(coefficients(glmsurvsize0)[1]+ 5*coefficients(glmsurvsize0)[2])))
survprob6 <- 1/(1+exp(-(coefficients(glmsurvsize0)[1]+ 6*coefficients(glmsurvsize0)[2])))
...
newdat <- data.frame(size=seq(min(survdat$size),max(survdat$size), length.out = 100))
newdat <- cbind(newdat, predict(glmsurvsize0, newdata = newdat, type = "link", se.fit = TRUE))
...
#or with polygon:
polygon(x=c(newdat$size,rev(newdat$size)), y=c(newdat$ciu,rev(newdat$cil)), col = rgb(0.8,0,0,0.2), border =NA)
...
\begin{Exercise}[difficulty=2, title={Genetic risk?}]
...
y <- -2 + atcor*0.95
popcollapse <- sapply(y, FUN=function(x) rbinom(1, 1, prob = 1/(1+exp(-x))))
plot(atcor, popcollapse)
summary(glm(popcollapse~atcor, family = "binomial"))

decline <- data.frame(popcollapse=popcollapse, pheno_autocorr=atcor)

decline$futurecollapse <- sapply(-2 + atcor*0.95, FUN=function(x) rbinom(1, 1, prob = 1/(1+exp(-x))))
decline$futurecollapse[decline$popcollapse==1] <- NA

decline$variance <- rnorm(200,6,1)+decline$popcollapse
write.csv(decline, file = "decline.csv", quote = FALSE, row.names = FALSE)
...
summary(glmcoll0 <- glm(popcollapse~pheno_autocorr, data = decline, family = "binomial"))
newdat <- data.frame(pheno_autocorr=seq(-1,1,length.out = 100))
newdat <- cbind(newdat, pred = predict(glmcoll0, type = "response", newdata = newdat))
...
<<>>=
sum(decline$futurecollapse[decline$pheno_autocorr>=0.5], na.rm=TRUE)
@

and 116 safe populations:
<<>>=
sum(1-decline$futurecollapse[decline$pheno_autocorr<0.5], na.rm=TRUE)
@
...
plot.roc(predictor = predict(glmcoll0, type = "response"), x = decline$popcollapse)
@

Adding another predictor helps improve the prediction:
<<>>=
glmcoll1 <- glm(popcollapse~pheno_autocorr+variance, data = decline, family = "binomial")

plot.roc(predictor = predict(glmcoll1, type = "response"), x = decline$popcollapse)
@
...
dat$y <- 1.9 + idint[dat$id] + dat$x*(-0.2) + rnorm(nrow(dat), 0, 0.2) + rnorm(n = 5, 0, 0.15)[dat$year-2012]

plot(dat$x, dat$y)
dat$bin <- sapply(dat$y, FUN=function(x){rbinom(n = 1, 1, prob = 1/(1+exp(-x)))})

plot(dat$x, dat$bin)
library(lme4)
summary(glmer(bin ~ 1 + x +(1|id) + (1|year), data = dat, family = "binomial"))

names(dat) <- c("repeats", "individual", "distance", "year", "y", "RunAway")
dat <- dat[,c("repeats", "individual", "distance", "year", "RunAway")]

write.csv(dat, file = "runaway.csv", quote = FALSE, row.names = FALSE)
@
